...
/*
** A hash table consists of an array of these buckets. Each bucket holds
** a copy of the key, a pointer to the data associated with the key, and
** a pointer to the next bucket that collided with this one, if there
** was one.
*/
...
/*
** Goes through a hash table and calls the function passed to it for
** each node that has been inserted. The function is passed a pointer to
** the key, and a pointer to the data associated with it.
*/
void enumerate(struct hash_table *table, void (*func)(char *,void *));

/*
** Frees a hash table. For each node that was inserted in the table, it
** calls the function whose address it was passed, with a pointer to the
** data that was in the table. The function is expected to free the
** data. Typical usage would be: free_table(&table, free) if the data
** placed in the table was dynamically allocated, or free_table(&table,
** NULL) if not. If the parameter passed is NULL, it knows not to call
