...
//
// This file has been inspected, reformatted, and lightly modified by
// Peter Chapin. Do not redistribute.
//

//**************************************
// Name: Hash table functions
// Description: Hash table management by Jerry Coffin, with improvements
// by HenkJan Wolthuis.
...
// http://www.Planet-Source-Code.com/xq/ASP/txtCodeId.710/lngWId.3/qx/vb/scripts/ShowCode.htm
// for details.
//**************************************
...
**
** Tested with Visual C 1.0 and Borland C 3.1. Compiles without
** warnings, and seems like it should be pretty portable.
*/

/*
** These are used in freeing a table. Perhaps I should code up something
** a little less grungy, but it works, so what the heck.
*/
static void (*function)(void *) = (void (*)(void *))NULL;
static hash_table *the_table = NULL;

/* Initialize the hash_table to the size asked for. Allocates space for
** the correct number of pointers and sets them to NULL. If it can't
** allocate sufficient memory, signals error by setting the size of the
** table to 0.
*/
...
/*
** Insert 'key' into hash table. Returns pointer to old data associated
** with the key, if any, or NULL if the key wasn't in the table
** previously.
*/
void *insert(char *key, void *data, hash_table *table)
{
  ...
  ** code would be moved into the loop above, and the insertion would
...
/*
** Look up a key and return the associated data. Returns NULL if the key
** is not in the table.
*/
...
/*
** Delete a key from the hash table and return associated data, or NULL
** if not present.
*/
      ...
      if (0 == strcmp(key, ptr -> key))
        {
          ...
          ** delete the first node in the list. This simply consists of
          ** putting our own 'next' pointer in the array holding the
          ** head of the list. We then dispose of the current node as
          ** above.
          */
          else
            {
...
/*
** free_table iterates the table, calling this repeatedly to free each
** individual node. This, in turn, calls one or two other functions -
** one to free the storage used for the key, the other passes a pointer
** to the data back to a function defined by the user, process the data
** as needed.
*/
